...
class TypeOfOperationDialog(QDialog, Ui_TypeOfOperationDialog):
    ...
    def __init__(self, parent=None):
        ...
        self.selectMission.clicked.connect(self.enter_new_operation)
        ...
        self.buttonBox_operation.accepted.connect(self.accept)  # OK button closes dialog
        self.buttonBox_operation.rejected.connect(self.reject)  # Cancel button closes dialog
    ...
    def enter_new_operation(self):
        """Handles custom user input for a new operation type."""
    ...
    def new_db(self):
        ...
        db_name = self.operation_enter.text() + ".db"
        if db_name.strip() == ".db":
            self.operation_enter.setStyleSheet("border: 1px solid red;")
        else:
            Db.modify_db_config(db_name)
    ...
    def open_flight_flow_window(self):
        """Opens the flight flow window or main application window."""
        QMessageBox.information(self, "Flight Flow", "Opening Flight Flow Main Window.")
        self.close()      
    def delete_operation(self):
        """Deletes the selected operation from the database, updates the combo box and the JSON file."""
        operation_type = self.operation_select.currentText()  # Get the selected operation
    
        if operation_type:
           # Remove from the database
            Db.delete_operation(operation_type)  # Call the method to delete from the database

        # Remove from the operations list (in-memory)
            if operation_type in self.operations:
                self.operations.remove(operation_type)

        # Remove from the combo box
            index = self.operation_select.findText(operation_type)
            if index != -1:
                self.operation_select.removeItem(index)

            # Update the JSON file to reflect the deletion
            with open(self.operations_file, "w") as file:
                json.dump(self.operations, file)

            QMessageBox.information(self, "Operation Deleted", f"Operation '{operation_type}' deleted successfully.")
